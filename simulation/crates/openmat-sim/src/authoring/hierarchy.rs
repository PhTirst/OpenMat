...
pub(crate) fn flatten(model: &Model) -> Result<Model, ModelError> {
    ...
    let mut resolve = |source: &Port| -> Result<Port, ModelError> {
    ...
    for b in &mut flat.blocks {
        b.parent = None;
        if matches!(b.kind, BlockKind::Outport { .. }) {
...
fn validate_hierarchy<'a>(
    ...
    for ((parent, input), ports) in &boundaries {
        let expected = match parent.and_then(|id| blocks.get(id)).map(|b| &b.kind) {
            Some(BlockKind::Subsystem { inputs, outputs }) => {
                if *input {
                    *inputs
                } else {
                    *outputs
    ...
    for b in &model.blocks {
        if let BlockKind::Subsystem { inputs, outputs } = b.kind {
